...
pub struct HasOneThrough<Owner: Model, Final: Model, Intermediate: Model> {
    owner_key: Value,
...
impl<Owner: Model, Final: Model, Intermediate: Model> HasOneThrough<Owner, Final, Intermediate> {
    ...
    pub async fn get(&self) -> Result<Option<Final>> { self.query().first().await }
